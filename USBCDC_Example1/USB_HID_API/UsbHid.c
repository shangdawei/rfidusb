...
|                                                                             |
+-----------------------------------------------------------------------------+
|  Source: UsbHid.c, v1.18 2009/06/11                                         |
|  Author: RSTO                                                               |
|                                                                             |
|  WHO          WHEN         WHAT                                             |
|  ---          ----------   ------------------------------------------------ |
...
BYTE USBHID_bytesInUSBBuffer(BYTE intfNum)
{
    ...
    if (HidReadCtrl.nBytesInEp > 0)         // If a RX operation is underway, part of data may was read of the OEP buffer
    {
        ...
        {
            bTmp2 = *(HidReadCtrl.pEP2+1);  // holds how many valid bytes in the EP buffer
            if (bTmp2 > (*HidReadCtrl.pCT2 & 0x7F) -2) // check if all data received correctly
            {
                bTmp1 += (*HidReadCtrl.pCT2 & 0x7F) -2;
            }
            else
            {
                bTmp1 += bTmp2;
    ...
    else
    {
        if (tOutputEndPointDescriptorBlock[EDB(HID_OUTEP_ADDR)].bEPBCTX & EPBCNT_NAK) //this buffer has a valid data packet
        {
            bTmp2 = tOutputEndPointDescriptorBlock[EDB(HID_OUTEP_ADDR)].bEPBCTX & 0x7F;
            bTmp1 = *((BYTE*)OEP1_X_BUFFER_ADDRESS+1);
        ...
        {
            bTmp2 = tOutputEndPointDescriptorBlock[EDB(HID_OUTEP_ADDR)].bEPBCTY & 0x7F;
            if (bTmp2-2 > *((BYTE*)OEP1_Y_BUFFER_ADDRESS+1)) // check if the count (second byte) is valid
            {
                bTmp1 += *((BYTE*)OEP1_Y_BUFFER_ADDRESS+1);
            }
            else
            {
                bTmp1 += bTmp2 - 2;
