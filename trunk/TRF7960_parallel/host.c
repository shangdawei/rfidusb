...
void USARTset(void)  //Uses USCI_B0
{
  ...
  UCB1CTL1 |= UCSWRST;                     // Disable USCI first
  UCB1CTL0 |= UCCKPH + UCMSB + UCMST + UCSYNC;  // 3-pin, 8-bit SPI master
  UCB1CTL1 |= UCSSEL_2;                     // SMCLK

  UCB1BR0 = 0;
  UCB1BR1 = 0;
  P4SEL |= BIT1 + BIT2 + BIT3;                            // P3.1,3.2,3.3 UCB0SIMO,UCB0SOMI,UCBOCLK option select

  TRF_SS_INIT();     // P3.0 - Slave Select
  TRF_SS_HIGH();    // Slave Select - inactive ( high)
 ...
 // IE2 |= UCB0RXIE + UCB0TXIE;	/* transmitter and reciever enable */
  ...
  //Harsha - code added
 // UCB1IFG &= ~UCRXIFG; //Clear Interrupt flag first
...
unsigned char Get_line(unsigned char *pline)
{
	...
	if (POLLING==0)	LPM0;										/* sync, wakeup by irq */
	while(rxdata != '1')
	{
	}
...
//	{
		...
		{					/* Firmware Version Number */
			phello = "Firmware Version 03_15_2010_LED_Control+ \r\n";
			send_cstring(phello);
		}
		else if(*pbuf == 0xF3)
		{					/*LED open on*/
